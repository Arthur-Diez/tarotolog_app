...
const COUNTRY_LABEL_MAP: Record<string, string> = Object.fromEntries(COUNTRY_OPTIONS.map((item) => [item.code, item.label]));

export function getCountryLabelEnglish(code: string | null | undefined): string {
  if (!code) return "Unknown";
  const normalized = code.trim().toUpperCase();
  if (!normalized) return "Unknown";
  return COUNTRY_LABEL_MAP[normalized] ?? normalized;
}
